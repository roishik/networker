...
export default function ContactCard({ contact, onClick }: ContactCardProps) {
  ...
  const getStatusColor = (contact: Contact) => {
    ...
    if (days <= 14) return "bg-green-400";
    if (days <= 60) return "bg-blue-400";
  ...
  return (
    ...
    >
      <div className="flex items-start space-x-3">
        <div
          className={`w-12 h-12 bg-gradient-to-br ${getRandomGradient(contact.id)} rounded-full flex items-center justify-center text-white font-semibold`}
        >
          {getInitials(contact)}
        </div>
        ...
        <div className="flex flex-col items-center space-y-1">
          ...
          <div
            className={`w-2 h-2 ${getStatusColor(contact)} rounded-full`}
