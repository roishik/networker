...
export function parseNoteText(text: string): ParsedNote {
  ...
  const companyPatterns = [
    ...
    // Company forms like "X Technologies", "X Capital", etc.
  ...
  for (const pattern of jobTitlePatterns) {
    ...
    if (match) {
      ...
      const job = match[0].toLowerCase();
      result.jobTitle = job.charAt(0).toUpperCase() + job.slice(1);
      // Remove the job title from the remaining text
      remainingText = remainingText.replace(match[0], "").trim();
      break;
    }
  }
